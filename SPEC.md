...
| ID | 要求 | 優先度 |
|---|---|---|
| F-01 | `src/sources.py` に 23 社の取得経路(feed / html / browser / pending)を宣言的に保持する。html は一覧からのリンク採取+記事ページからの日付・タイトル抽出の統一方式 | must |
...
- **全 23 社の取得経路が確立**(2026-08-21 実測)。うち browser 戦略 2 社(Zhipu・Tencent)は CI でのみ実取得
- 経路は腐る前提(2026-08 の調査で 1 社移転・1 社ドメイン消滅)。健全性判定と月次点検で「気づかないまま古い内容を見せ続ける」ことを防ぐ
